...
export type SpeakerRole = 'narrator' | 'patient' | 'doctor';
...
export const DEFAULT_SPEAKER_PROFILES: Record<SpeakerRole, SpeakerProfile> = {
  narrator: {
    role: 'narrator',
    rate: 1.0,
    pitch: 0.9, // slightly lower for clinical preceptor
    volume: 1.0,
  },
  patient: {
    role: 'patient',
    rate: 1.0,
    pitch: 1.15, // slightly higher for natural female voice
    volume: 1.0,
  },
  doctor: {
    role: 'doctor',
...
/**
 * Speak text using a specific speaker profile (for multi-voice scenes).
 * Merges the speaker profile over the base config.
 */
export async function speakAs(
  ...
  const merged: TTSConfig = {
    ...config,
    voiceId: profile.voiceId ?? config.voiceId,
    rate: profile.rate,
    pitch: profile.pitch,
    volume: profile.volume,
  };
  return speak(text, merged, options);
}
