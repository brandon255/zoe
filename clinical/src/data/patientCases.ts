...
export interface PatientCase {
  ...
  notes?: string;
  /** Adult intimate anatomy/training is opt-in per case. */
...
export const BUILTIN_CASES: PatientCase[] = [
  mkCase({
    ...
    name: 'Zoe — Annual GYN (Friday demo)',
    title: '28F annual well-woman visit',
    specialty: 'gynecology',
    chiefComplaint: "I'm here for my yearly exam.",
    history:
      '28-year-old female, G0, generally healthy, regular cycles on OCP. Cooperative with step-by-step gyn exam coaching (gown → table → stirrups → lithotomy → speculum → inspect). Partner may be present for support when invited — clinical only.',
    attachedObjects: {
      glasses: false,
      stethoscope: false,
      gown: true,
    ...
    defaultLayers: { skin: true, organs: true, pelvis: true },
    suggestedCommands: [
      'Put on the gown',
      'Lie on the table',
    ...
    customFigure: {
      ...
      age: 28,
      background:
        '28-year-old female, G0, annual gyn visit. Follows doctor directions for positioning and exam.',
      voice:
        'Calm, cooperative, short answers. Asks for step-by-step coaching. Clinical tone only.',
    },
  }),
