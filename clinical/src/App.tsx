...
import { FigureSourceSelector } from './components/FigureSourceSelector';
import { CaseLibrary } from './components/CaseLibrary';
import { MeshGenPanel } from './components/MeshGenPanel';
import { useVoiceCommands } from './hooks/useVoiceCommands';
import { useConversation } from './hooks/useConversation';
import { useEncounter } from './hooks/useEncounter';
import { DEFAULT_SPEAKER_PROFILES } from './services/tts';
...
import type { VoiceIntent, LayerId, FigureSource, ClinicalInsertionProp } from './types';
...
type Mode = 'command' | 'conversation' | 'encounter';
...
export default function App() {
  ...
  const [figureSource, setFigureSource] = useState<FigureSource>('procedural');
  ...
  // Adult clinical-training insertion prop
  const [foreignBody, setForeignBody] = useState<ClinicalInsertionProp | null>(null);
  ...
  /** Bridge so applyCommandIntent can toggle encounter tray before the hook exists in the dep graph */
  const encounterToolBridge = useRef<((tool: ExamTool, active: boolean) => void) | null>(null);
  ...
  const [currentCaseId, setCurrentCaseId] = useState<string | null>('zoe-annual-gyn');
  ...
  const [husbandPresent, setHusbandPresent] = useState(false);
  const [husbandCamActive, setHusbandCamActive] = useState(false);
  const [husbandCamPhase, setHusbandCamPhase] = useState<string>('idle');
  ...
  const [llmConfig, setLLMConfigState] = useState<LLMConfig>(() => ({
    ...loadLLMConfig(),
    systemPrompt: MEDSTAGE_PATIENT_PROMPT,
  }));
  ...
  // Voice command hook — atlas/tray intents apply in every mode; anatomy cmds in command mode
  const voice = useVoiceCommands({
    onIntent: (intent) => {
      const stageIntents = new Set([
        'setAtlasView',
        'setExamTool',
        'setHusbandPresent',
        'husbandTakeCamera',
      ...
      if (mode === 'command' || stageIntents.has(intent.kind)) {
  ...
  const applyCommandIntent = useCallback((intent: VoiceIntent) => {
    ...
    switch (intent.kind) {
      ...
      case 'endoscope': {
        if (!intimateAnatomyEnabled) {
          setLastAction('Load an adult clinical case to enable endoscope training');
          break;
        }
      ...
      case 'insertForeignBody': {
        if (!intimateAnatomyEnabled) {
          setLastAction('Load an adult clinical case to enable insertion training');
          break;
        }
      ...
      case 'setAtlasView': {
        ...
        setAtlasSlot(intent.view);
        if (intent.view === 'lithotomy') {
          setIdleRotate(false);
          setModelRotationX(0.35);
        }
        setLastAction(`Atlas: ${intent.view}`);
        break;
      }
      case 'setExamTool': {
        const tool = intent.tool as ExamTool;
        setDemoExamTools((prev) => {
          const next = new Set(prev);
          if (intent.active) {
            next.add(tool);
            if (tool === 'speculum_open') next.add('speculum');
            if (tool === 'speculum') next.delete('speculum_open');
          } else {
            next.delete(tool);
            if (tool === 'speculum') next.delete('speculum_open');
          }
        ...
        // Encounter mode shares the same tray via hook when available
        encounterToolBridge.current?.(tool, intent.active);
        if (intent.active && (tool === 'speculum' || tool === 'speculum_open')) {
          setAtlasSlot(tool === 'speculum_open' ? 'cervix-speculum' : 'pelvis-external');
          setAtlasVisible(true);
        }
        setLastAction(`${intent.active ? 'Using' : 'Put away'} ${intent.tool}`);
        break;
      }
      case 'setHusbandPresent': {
        setHusbandPresent(intent.present);
        if (intent.present) setAtlasVisible(true);
        if (!intent.present) {
          setHusbandCamActive(false);
          setHusbandCamPhase('idle');
        }
        setLastAction(intent.present ? 'Partner present (support)' : 'Partner dismissed');
        break;
      }
      case 'husbandTakeCamera': {
        if (!intimateAnatomyEnabled) {
          setLastAction('Load Zoe adult clinical case first');
          break;
        }
        if (intent.active) {
          setHusbandPresent(true);
          setHusbandCamActive(true);
          setIdleRotate(false);
          setEndoscopeView('vaginal');
          // Speculum path helps the training narrative
          setDemoExamTools((prev) => {
            const next = new Set(prev);
            next.add('gloves');
            next.add('speculum');
            next.add('speculum_open');
            next.add('light');
            return next;
          });
          encounterToolBridge.current?.('speculum_open', true);
          setAtlasSlot('cervix-speculum');
          setLastAction('Husband cam — entering canal toward cervix');
        } else {
          setHusbandCamActive(false);
          setHusbandCamPhase('idle');
          setEndoscopeView(null);
          setCameraTarget([0, activeFigure.proportions.waistY - activeFigure.proportions.footHeight, 0]);
          setCameraDistance(3.5);
          setLastAction('Exited husband cam');
        }
        break;
      }
      case 'loadCase': {
  ...
  }, [intimateAnatomyEnabled, activeFigure]);
  ...
  // Encounter hook — used when the user wants to practice a patient encounter
  // with an LLM-driven patient who responds in real time.
  const encounter = useEncounter({
    enabled: mode === 'encounter',
    llmConfig,
    ttsConfig,
    ...
    transcriptFinal: voice.transcriptFinal,
    onTranscriptHandled: voice.ackTranscript,
    onAnatomyMentioned: (layers) => {
      setLastAction(`Anatomy mentioned: ${layers.join(', ')}`);
    },
  });

  useEffect(() => {
    encounterToolBridge.current = encounter.setTool;
  }, [encounter.setTool]);

  // Sync atlas to encounter phase during Friday demo spine
  useEffect(() => {
    if (mode !== 'encounter') return;
    setAtlasSlot(atlasSlotForPhase(encounter.currentPhase));
    setAtlasVisible(true);
  }, [mode, encounter.currentPhase]);

  // Sync encounter phase state with hook
  // (We track currentPhase inside the hook, but expose it for the UI.)
  useEffect(() => {
    setLastAction((curr) => {
      if (mode !== 'encounter') return curr;
      if (encounter.phase === 'thinking') return 'Patient is thinking…';
      if (encounter.phase === 'patient-speaking') return 'Patient is speaking…';
      if (encounter.phase === 'error' && encounter.lastError) {
        return `LLM error: ${encounter.lastError}`;
      }
      return curr;
    });
  }, [mode, encounter.phase, encounter.lastError]);
  ...
  const handleLoadCase = useCallback((c: PatientCase) => {
    ...
    setAtlasSlot('full-front');
    setAtlasVisible(true);
    setHusbandPresent(false);
    setHusbandCamActive(false);
    setHusbandCamPhase('idle');
    setDemoExamTools(new Set(['gloves']));
  ...
  useEffect(() => {
    const zoe = getCaseById('zoe-annual-gyn');
    if (zoe) handleLoadCase(zoe);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- Friday default once
  }, []);
      ...
      >
        <Suspense fallback={null}>
          <Scene
            ...
            intimateAnatomyEnabled={intimateAnatomyEnabled}
            examTools={
              mode === 'encounter'
                ? encounter.activeTools
                : demoExamTools.size > 0
                  ? demoExamTools
                  : undefined
            }
            ...
            husbandCamActive={husbandCamActive}
            onHusbandCamPhase={(phase) => {
              setHusbandCamPhase(phase);
              if (phase === 'cervix') setLastAction('Husband cam — cervix in view');
              else if (phase === 'canal') setLastAction('Husband cam — in canal');
              else if (phase === 'entering') setLastAction('Husband cam — entering');
            }}
          />
        </Suspense>
      </Canvas>

      {!welcomeVisible && husbandCamActive && (
        <div className="husband-cam-hud" role="status">
          <div className="husband-cam-hud-title">Husband cam</div>
          <div className="husband-cam-hud-phase">
            {husbandCamPhase === 'cervix'
              ? 'Cervix in view'
              : husbandCamPhase === 'canal'
                ? 'Advancing through canal'
                : husbandCamPhase === 'entering'
                  ? 'Entering vaginal canal'
                  : 'Live'}
          </div>
          <button
            type="button"
            className="husband-cam-exit"
            onClick={() =>
              applyCommandIntent({ kind: 'husbandTakeCamera', active: false })
            }
          >
            Exit canal view
          </button>
        ...
        <>
          <div className="mode-toggle">
            <button
              ...
              title="Voice commands — speak or type structured actions"
            >
              Command
            </button>
            <button
              ...
              title="Free-form conversation with the patient"
            >
              Chat
            </button>
            <button
              className={mode === 'encounter' ? 'active' : ''}
              onClick={() => setMode('encounter')}
              title="Patient encounter simulation — practice the full exam with an LLM-driven patient"
            >
              Encounter
            </button>
            <button
              ...
              onClick={() => setAtlasVisible((v) => !v)}
              title="Toggle Zoe anatomy atlas"
            >
              Atlas
            </button>
          </div>
          ...
          <FigureSourceSelector
            currentSource={figureSource}
            onSourceChange={setFigureSource}
            onCustomGlbLoad={setCustomGlbUrl}
            visible={true}
          />
          ...
          {mode === 'encounter' && (
            <EncounterMode
              ...
              isListening={displayState === 'listening' && mode === 'encounter'}
              voiceTranscript={voice.transcript}
              voiceTranscriptFinal={voice.transcriptFinal}
              onToggleVoice={handleVoiceToggle}
              voiceSupported={voice.isSupported}
              onSendDoctorMessage={encounter.doctorSays}
              onEndEncounter={() => setMode('command')}
              onReset={encounter.reset}
              onAdvancePhase={encounter.advancePhase}
              onSetPhase={encounter.setPhaseByIndex}
              lastError={encounter.lastError}
              scorecard={encounter.summary}
              activeTools={encounter.activeTools}
              onToolToggle={encounter.setTool}
            />
          )}
...
function intentLabel(intent: VoiceIntent): string {
  switch (intent.kind) {
    ...
    case 'setAtlasView':
      return `atlas ${intent.view}`;
    case 'setExamTool':
      return `${intent.active ? 'use' : 'put away'} ${intent.tool}`;
    case 'setHusbandPresent':
      return intent.present ? 'partner present' : 'partner leave';
    case 'husbandTakeCamera':
      return intent.active ? 'husband cam on' : 'husband cam off';
    case 'reset':
      return 'reset';
    case 'help':
      return 'help';
