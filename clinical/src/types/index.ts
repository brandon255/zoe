...
export type FigureSource = 'procedural' | 'glb-cesium' | 'glb-custom';

export type ClinicalInsertionProp = 'training_insert' | 'forceps' | 'thermometer';
...
export type VoiceIntent =
  ...
  | { kind: 'exitEndoscope' }
  | { kind: 'insertForeignBody'; object: ClinicalInsertionProp }
  | { kind: 'removeForeignBody' }
  | { kind: 'setAtlasView'; view: AtlasViewId }
  | { kind: 'setExamTool'; tool: ExamToolIntentId; active: boolean }
  | { kind: 'setHusbandPresent'; present: boolean }
  | { kind: 'husbandTakeCamera'; active: boolean }
  | { kind: 'help' }
