...
interface PelvicCavitiesProps {
  ...
  /** Optional foreign body inside the canal/rectum (e.g., cucumber, forceps) */
  foreignObject?: 'cucumber' | 'forceps' | 'thermometer' | null;
...
function VaginalCanal({
  ...
  return (
    <group position={[startX, startY, startZ]} rotation={[Math.PI - angle, 0, 0]}>
      ...
      {/* Cervical os (at the top/deep end) */}
...
function Rectum({
  ...
  return (
    <group position={[startX, startY, startZ]}>
      ...
      />
      {/* Foreign object (the cucumber) */}
      {foreignObject === 'cucumber' && (
        <Cucumber
...
function Cucumber({ position }: { position: THREE.Vector3 }) {
  return (
    <group position={position}>
      {/* Cucumber body */}
      <mesh castShadow>
        <capsuleGeometry args={[0.018, 0.12, 8, 16]} />
        <meshStandardMaterial color="#4a6a2a" roughness={0.6} />
      </mesh>
